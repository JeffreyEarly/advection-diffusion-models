...
figure(Color="w", Units="points", Position=[50 50 figureWidthMedium 380]);
tl = tiledlayout(3, 1, TileSpacing="compact", Padding="compact");

ax1 = nexttile;
fillRankedEnvelope(ax1, tBootstrapDays, sigma(:, indices90)/f0, errorColor, errorAlpha, stdEdgeColor);
fillRankedEnvelope(ax1, tBootstrapDays, sigma(:, indices68)/f0, errorColor, errorAlpha, stdEdgeColor);
hold(ax1, "on");
plot(ax1, tBootstrapDays, sigma(:, indexBest)/f0, LineWidth=meanLineWidth);
...
box(ax1, "on");
title(ax1, "Site 1 gridded streamfunction bootstrap fit", FontSize=figureTitleSize, FontName=figureFont);

ax2 = nexttile;
fillRankedEnvelope(ax2, tBootstrapDays, thetaDegrees(:, indices90), errorColor, errorAlpha, stdEdgeColor);
fillRankedEnvelope(ax2, tBootstrapDays, thetaDegrees(:, indices68), errorColor, errorAlpha, stdEdgeColor);
hold(ax2, "on");
plot(ax2, tBootstrapDays, thetaDegrees(:, indexBest), LineWidth=meanLineWidth);
...
ax3 = nexttile;
fillRankedEnvelope(ax3, tBootstrapDays, uCenter(:, indices90), errorColor, errorAlpha, stdEdgeColor);
fillRankedEnvelope(ax3, tBootstrapDays, uCenter(:, indices68), errorColor, errorAlpha, stdEdgeColor);
fillRankedEnvelope(ax3, tBootstrapDays, vCenter(:, indices90), errorColor, errorAlpha, stdEdgeColor);
fillRankedEnvelope(ax3, tBootstrapDays, vCenter(:, indices68), errorColor, errorAlpha, stdEdgeColor);
hold(ax3, "on");
uLine = plot(ax3, tBootstrapDays, uCenter(:, indexBest), LineWidth=meanLineWidth);
vLine = plot(ax3, tBootstrapDays, vCenter(:, indexBest), LineWidth=meanLineWidth);
xlabel(ax3, "time (days)", FontSize=figureAxisLabelSize, FontName=figureFont);
ylabel(ax3, "u_c/v_c (m/s)", FontSize=figureAxisLabelSize, FontName=figureFont);
xlim(ax3, [tBootstrapDays(1), tBootstrapDays(end)]);
ylim(ax3, [-0.25 0.25]);
legend(ax3, [uLine, vLine], "u_c", "v_c", Location="southwest", FontSize=figureLegendSize, NumColumns=2);
set(ax3, FontSize=figureAxisTickSize, FontName=figureFont);
box(ax3, "on");

title(tl, "Site 1 bootstrap parameter time series", FontSize=figureTitleSize, FontName=figureFont);

if shouldSaveFigures == 1
    print(fullfile(scriptDir, "Site1BootstrapParameters.eps"), "-depsc2");
end
...
title(axMeso, "Site 1 fixed-frame mesoscale trajectories", FontSize=figureTitleSize, FontName=figureFont);
set(axMeso, FontSize=figureAxisTickSize, FontName=figureFont);
box(axMeso, "on");

if shouldSaveFigures == 1
    print(fullfile(scriptDir, "Site1BootstrapDecompFigA.eps"), "-depsc2");
end
...
title(tlDecomp, "Site 1 fixed-frame decomposition", FontSize=figureTitleSize, FontName=figureFont);

if shouldSaveFigures == 1
    print(fullfile(scriptDir, "Site1BootstrapDecompFigB.eps"), "-depsc2");
end
...
title(tlCom, "Site 1 COM-frame trajectories and RMS speeds", FontSize=figureTitleSize, FontName=figureFont);

if shouldSaveFigures == 1
    print(fullfile(scriptDir, "Site1BootstrapDecompFigC.eps"), "-depsc2");
end
