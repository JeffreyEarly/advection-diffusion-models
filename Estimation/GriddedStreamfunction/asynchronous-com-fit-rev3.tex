...
The center-of-mass (COM) trajectory is defined by a linear spline smoother, so the COM
...
The only mean-free assumption is that the mesoscale contribution to the particle evolution
in the COM frame has zero mean velocity.
...
\{B_q(t)\}_{q=1}^{Q_b}.
\end{equation}
\textcolor{red}{I guess $Q_b$ is the number of basis splines?}
...
\qquad
\dot{\mathbf{m}}_x=S\dot{\mathbf{x}},
\qquad
\dot{\mathbf{m}}_y=S\dot{\mathbf{y}}.
\label{eq:S_com_relation}
\end{equation}

\textcolor{red}{The velocity is actually computed from the derivative of the BSpline, not the smoothing matrix of the applied the derivative of the observations.}
...
\textcolor{red}{In practice the observations, e.g. $\mathbf{x}$, are stored in an interpolating spline with a time domain that will generally be smaller than $\hat{\mathbf{u}}^{\mathrm{bg}}$ and  $\hat{\mathbf{u}}^{\mathrm{meso}}$. The submeoscale signal should thus use the same basis splines as the observations themselves, computed by formulating an interpolating spline at the observation points.}
...
y^{\mathrm{bg}}(t_0)=0.
\end{equation}
In practice, $\hat{\mathbf{u}}^{\mathrm{bg}}$ may be projected onto the fast
spline basis $\mathcal{B}$ to obtain a smooth representation for integration.
...
\begin{equation}
x_k^{\mathrm{meso}}(t_{k,0})=0,
\qquad
y_k^{\mathrm{meso}}(t_{k,0})=0.
\end{equation}
...
x_k(t_{k,0})
+
\bigl[x^{\mathrm{bg}}(t)-x^{\mathrm{bg}}(t_{k,0})\bigr]
+
x_k^{\mathrm{meso}}(t)
+
x_k^{\mathrm{sm}}(t),
...
y_k(t_{k,0})
+
\bigl[y^{\mathrm{bg}}(t)-y^{\mathrm{bg}}(t_{k,0})\bigr]
+
y_k^{\mathrm{meso}}(t)
+
y_k^{\mathrm{sm}}(t).
...
\hat{\mathbf{v}}^{\mathrm{sm}}.
\end{equation}
Defining trajectory components by integration from $t_{k,0}$,
\begin{equation}
\tilde{x}_k^{\alpha}(t)
=
\int_{t_{k,0}}^{t} \tilde{u}_k^{\alpha}(s)\,ds,
\qquad
\tilde{y}_k^{\alpha}(t)
=
\int_{t_{k,0}}^{t} \tilde{v}_k^{\alpha}(s)\,ds,
\qquad
\alpha \in \{\mathrm{meso},\mathrm{sm}\},
\end{equation}
we obtain
\begin{equation}
\tilde{x}_k(t)
...
\subsection{Zero gauge (copied from earlier documentation}
 ...
 coefficient space. Let $c_k \in \mathbf{R}^{M_q M_r}$ be the
